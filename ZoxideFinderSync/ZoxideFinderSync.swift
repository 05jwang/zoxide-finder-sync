...
class FinderObserver: NSObject {
  ...
  private func attachObserver(to pid: pid_t) {
    ...
    guard result == .success, let observer = newObserver else {
      Task {
        await FileLogger.shared.log(
          "Failed to create AXObserver. Code: \(result.rawValue)", type: .error)
      }
      return
    }
    ...
    AXObserverAddNotification(
      observer, finderElement, kAXFocusedWindowChangedNotification as CFString, refcon)
    AXObserverAddNotification(
      observer, finderElement, kAXMainWindowChangedNotification as CFString, refcon)
    AXObserverAddNotification(
      observer, finderElement, kAXTitleChangedNotification as CFString, refcon)
  ...
  nonisolated private func runZoxideCommand(executableURL: URL, arguments: [String]) async
    -> String?
  {
    // Time Complexity: O(N) where N is the length of the command output.
    // Space Complexity: O(N) to store the output string in memory.
  ...
  private func getZoxideScore(executableURL: URL, for path: String) async -> Double {
    guard
      let output = await runZoxideCommand(
        executableURL: executableURL, arguments: ["query", "-s", path])
    else { return 0.0 }
  ...
  private func addZoxidePath(executableURL: URL, _ path: String) async {
    _ = await runZoxideCommand(executableURL: executableURL, arguments: ["add", path])
  }
  // MARK: - Path Evaluation

  private func isBlacklisted(path: String) -> Bool {
    for blacklistedPath in SettingsManager.shared.blacklist {
      if path == blacklistedPath || path.hasPrefix(blacklistedPath + "/") { return true }
    }
    return false
  }
  func evaluatePath() async {
    // We await the detached AppleScript execution
    guard let rawPath = await fetchFrontmostFinderPath(), !rawPath.isEmpty else { return }
    ...
    if currentPath != lastPath {
      ...
      if isBlacklisted(path: currentPath) {
        await FileLogger.shared.log("Ignored (Blacklisted): \(currentPath)")
        return
      }
      ...
      if SettingsManager.shared.isZoxideAddEnabled {
        guard let zoxideURL = getZoxideExecutableURL() else {
          await FileLogger.shared.log(
            "Error: Could not locate the zoxide executable.", type: .error)
          return
        }

        // Await the scores without locking up the UI
        let scoreBefore = await getZoxideScore(executableURL: zoxideURL, for: currentPath)
        await addZoxidePath(executableURL: zoxideURL, currentPath)
        let scoreAfter = await getZoxideScore(executableURL: zoxideURL, for: currentPath)
        let delta = scoreAfter - scoreBefore

        let logStr = String(
          format: "  -> Before: %5.2f | After: %5.2f | Change: %+.2f", scoreBefore, scoreAfter,
          delta)
  ...
  nonisolated private func fetchFrontmostFinderPath() async -> String? {
    // Time Complexity: O(1) bounded by inter-process communication with Finder.
    // Space Complexity: O(1) for the resulting script output.
    return await Task.detached {
      let scriptSource = """
      ...
      if let scriptObject = NSAppleScript(source: scriptSource) {
        ...
        if let error = error {
          Task { await FileLogger.shared.log("AppleScript Error: \(error)", type: .error) }
          return nil
        }
